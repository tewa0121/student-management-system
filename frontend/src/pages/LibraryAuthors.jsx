...
const API_URL = 'http://localhost:5000/api/library/authors';
const getAuthHeader = () => ({
...
const LibraryAuthors = () => {
  ...
  const handleEdit = (item) => {
    setForm({ name: item.name, biography: item.biography || '' });
    ...
    <div style={{ padding: 20 }}>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{ marginBottom: 20, padding: 20, border: '1px solid #ccc', borderRadius: 8 }}>
          <h3>{editingId ? 'Edit Author' : 'Add New Author'}</h3>
          <input name="name" placeholder="Author Name" value={form.name} onChange={handleChange} required style={{ width: '100%', padding: 8, marginBottom: 10 }} />
          <textarea name="biography" placeholder="Biography" value={form.biography} onChange={handleChange} rows={4} style={{ width: '100%', padding: 8 }} />
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ background: '#f0f0f0' }}>
            <th>ID</th><th>Name</th><th>Biography</th><th>Actions</th>
          </tr>
        </thead>
        <tbody>
          {items.map(item => (
